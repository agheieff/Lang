...
def track_word_exposure(
    ...
    try:
        for word_info in word_data:
            ...
            # Note: SRS parameters are updated when user interacts with words (clicks/views)
            # For pure exposure, we just track the event
...
def track_interactions_from_session(
    ...
    try:
        ...
        for interaction in interactions:
            ...
            # Update SRS parameters using SRS module
            srs_update = update_srs_from_session(
                lexeme=lexeme,
                was_clicked=clicked,
                click_count=click_count,
                was_viewed=translation_viewed,
            )

            # Apply SRS update
            lexeme.stability = srs_update.stability
            lexeme.alpha = srs_update.alpha
            lexeme.beta = srs_update.beta
            lexeme.difficulty = srs_update.difficulty
            lexeme.familiarity = srs_update.familiarity
            lexeme.importance = srs_update.importance
            lexeme.next_due_at = srs_update.next_due_at
