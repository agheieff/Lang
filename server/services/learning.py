...
def get_srs_cards(
    ...
    try:
        ...
        for lexeme in due_lexemes:
            ...
            variants = db.query(LexemeVariant).filter(
...
def get_review_stats(
    ...
    try:
        total_lexemes = db.query(Lexeme).filter(
            Lexeme.profile_id == profile_id,
        ).count()
        
        due_lexemes = db.query(Lexeme).filter(
            Lexeme.profile_id == profile_id,
            Lexeme.next_due_at <= datetime.now(timezone.utc),
        ).count()
        
        new_lexemes = db.query(Lexeme).filter(
            Lexeme.profile_id == profile_id,
            Lexeme.clicks == 0,
        ).count()
        
        learning_lexemes = db.query(Lexeme).filter(
            Lexeme.profile_id == profile_id,
            Lexeme.familiarity < 0.5,
        ).count()
        
        mature_lexemes = db.query(Lexeme).filter(
            Lexeme.profile_id == profile_id,
